...
.DESCRIPTION
  ...
  V1.0.2 additionally eliminates:
    APEX_PS51_DOCKER_GO_TEMPLATE_QUOTED_STRING_NATIVE_ARGUMENT_CORRUPTION

  Docker Go-template formatting is no longer used for inspect/service-census
  evidence. Container IDs are obtained with quote-free Docker CLI arguments and
  full `docker inspect` JSON is parsed inside Windows PowerShell. This removes
  quoted-string Go-template literals such as label lookups from the PS5.1 native
  argument boundary.
...
$ScriptVersion = 'V1_0_2'
$AuthorityId = 'APEX_WATANY_CONTROL_CENTER_STAGE_X_ERPNEXT_PHASE2_REDIS_RUNTIME_RECOVERY_EVIDENCE_FINALIZATION_PS51_NATIVE_ARGUMENT_AND_DOCKER_GO_TEMPLATE_TRANSPORT_REPAIR_PRE_BACKEND_GATE_CLOSURE_V1_0_2'
...
function Get-NetworkNamesFromInspect {
    ...
    $networksObject = $InspectObject.NetworkSettings.Networks
...
function Get-NetworkAliasesFromInspect {
    ...
    $aliasValues = @($networkProperty.Value.Aliases)
...
function Get-MountRowsFromInspect {
    ...
    foreach ($mountObject in @($InspectObject.Mounts)) {
        $rows.Add([pscustomobject]@{
            Type = [string]$mountObject.Type
            Name = [string]$mountObject.Name
            Source = [string]$mountObject.Source
            Destination = [string]$mountObject.Destination
            ReadWrite = [bool]$mountObject.RW
        })
    }
...
function Get-RedisRuntimeProof {
    ...
    $runningState = [string]$inspectObject.State.Status
    $imageName = [string]$inspectObject.Config.Image

    $healthText = 'NOT_DEFINED'
    if (($null -ne $inspectObject.State.Health) -and
        (-not [string]::IsNullOrWhiteSpace([string]$inspectObject.State.Health.Status))) {
        $healthText = [string]$inspectObject.State.Health.Status
    }
...
try {
    ...
    $dbState = [string]$dbInspectObject.State.Status
    $dbHealth = 'NOT_DEFINED'
    if (($null -ne $dbInspectObject.State.Health) -and
        (-not [string]::IsNullOrWhiteSpace([string]$dbInspectObject.State.Health.Status))) {
        $dbHealth = [string]$dbInspectObject.State.Health.Status
    }
    $dbImage = [string]$dbInspectObject.Config.Image
    ...
    $postRedisDbHealth = 'NOT_DEFINED'
    if (($null -ne $postRedisDbInspectObject.State.Health) -and
        (-not [string]::IsNullOrWhiteSpace([string]$postRedisDbInspectObject.State.Health.Status))) {
        $postRedisDbHealth = [string]$postRedisDbInspectObject.State.Health.Status
    }
    ...
    foreach ($runningContainerId in $runningContainerIds) {
        ...
        $containerName = ([string]$runningInspectObject.Name).TrimStart('/')
        $containerImage = [string]$runningInspectObject.Config.Image
        $containerStatus = [string]$runningInspectObject.State.Status

        $serviceName = ''
        $labelProperties = $runningInspectObject.Config.Labels
