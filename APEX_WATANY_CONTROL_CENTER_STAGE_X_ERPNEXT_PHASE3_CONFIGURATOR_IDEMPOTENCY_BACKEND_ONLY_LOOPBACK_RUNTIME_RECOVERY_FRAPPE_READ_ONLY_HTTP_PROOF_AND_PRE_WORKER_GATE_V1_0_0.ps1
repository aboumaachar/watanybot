...
.DESCRIPTION
  Resumes from the sealed Phase 2 PASS. It never replays Phase 2 recovery.
  It first proves configurator is unnecessary because the preserved global
  Frappe configuration is already converged. Only then may it start the backend
  service, using the Watany loopback override, and prove Frappe read-only HTTP
  behavior at 127.0.0.1:18080.
...
$AuthorityId = 'APEX_WATANY_CONTROL_CENTER_STAGE_X_ERPNEXT_PHASE3_CONFIGURATOR_IDEMPOTENCY_BACKEND_ONLY_LOOPBACK_RUNTIME_RECOVERY_FRAPPE_READ_ONLY_HTTP_PROOF_AND_PRE_WORKER_GATE_V1_0_0'
$ScriptVersion = 'V1_0_0'
...
function Start-BackendOnly {
    ...
    $resultObject = Invoke-Native -Executable $DockerPath -ArgumentVector $arguments -Label 'BACKEND_ONLY_START'
...
try {
    ...
    Write-Output 'BACKEND_START_REPLAYED=NO'
