...
if ([string]::IsNullOrWhiteSpace($RepoRoot)) {
    throw "RepoRoot is required when not running from a repo folder. Example: powershell -NoProfile -ExecutionPolicy Bypass -File `$env:USERPROFILE\Downloads\APEX-WhatsApp-Chat-Behavior-V5.ps1 -RepoRoot C:\xampp\htdocs\projectx\watanybot -Apply"
}
...
Set-Content -LiteralPath $LogPath -Value "APEX WhatsApp Chat Behavior V5`r`nRunId: $RunId`r`nRepoRoot: $RepoRoot`r`n" -Encoding utf8
...
Write-Step "APEX WhatsApp Chat Behavior V5 started"
...
try {
    ...
    if (-not $SkipSelfInstall -and $SelfPath -and $SelfPath.StartsWith($Downloads, [System.StringComparison]::OrdinalIgnoreCase)) {
        ...
        $DestScript = Join-Path $ScriptDir "APEX-WhatsApp-Chat-Behavior-V5.ps1"
...
$ChatMatches = $ChatMatches | Sort-Object RelativePath -Unique

Write-Step "Repository files scanned: $($RepoFiles.Count)"
Write-Step "Potential chat-related files found: $($ChatMatches.Count)"
...
$MatchedRows = @()
if ($ChatMatches.Count -gt 0) {
    foreach ($M in ($ChatMatches | Select-Object -First 250)) {
...
$Plan = $PlanTemplate.
    ...
    Replace("{{MATCH_COUNT}}", [string]$ChatMatches.Count).
...
$ReportTemplate = @'
# APEX WhatsApp Chat Behavior V5 Report
...
- V5 avoids StrictMode variable-expansion failures, generic List .Add() overload issues, and the PowerShell automatic $Matches variable collision.
...
Write-Step "APEX WhatsApp Chat Behavior V5 finished."
Write-Host ""
Write-Host "DONE"
