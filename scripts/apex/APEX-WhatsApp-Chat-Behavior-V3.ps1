...
if ([string]::IsNullOrWhiteSpace($RepoRoot)) {
    throw "RepoRoot is required when not running from a repo folder. Example: powershell -NoProfile -ExecutionPolicy Bypass -File `$env:USERPROFILE\Downloads\APEX-WhatsApp-Chat-Behavior-V3.ps1 -RepoRoot C:\xampp\htdocs\projectx\watanybot -Apply"
}
...
Set-Content -LiteralPath $LogPath -Value "APEX WhatsApp Chat Behavior V3`r`nRunId: $RunId`r`nRepoRoot: $RepoRoot`r`n" -Encoding utf8
...
Write-Step "APEX WhatsApp Chat Behavior V3 started"
...
try {
    ...
    if (-not $SkipSelfInstall -and $SelfPath -and $SelfPath.StartsWith($Downloads, [System.StringComparison]::OrdinalIgnoreCase)) {
        ...
        $DestScript = Join-Path $ScriptDir "APEX-WhatsApp-Chat-Behavior-V3.ps1"
...
$MatchedRows = New-Object System.Collections.Generic.List[string]
if ($Matches.Count -gt 0) {
    foreach ($M in ($Matches | Select-Object -First 250)) {
        $MatchedRows.Add("| ``$($M.RelativePath)`` | $($M.Reason) | $($M.SizeBytes) |")
    }
} else {
    $MatchedRows.Add("| _None found_ | - | - |")
}
...
# Optional scaffold
# -----------------------------
$ScaffoldFiles = New-Object System.Collections.Generic.List[string]

if ($PatchScaffold) {
    if (-not [string]::IsNullOrWhiteSpace($FrontendRoot)) {
        Write-Step "PatchScaffold enabled"
        $CreatedFiles = New-ChatScaffold -FrontendRoot $FrontendRoot -BackupDir $BackupDir
        foreach ($C in $CreatedFiles) { $ScaffoldFiles.Add($C) }
    } else {
...
if ($RunValidation) {
    if (Test-Path -LiteralPath $PackageJsonPath) {
        if (Has-NpmScript -PackageJson $PackageJson -ScriptName "typecheck") {
            $ValidationResults.Add((Invoke-ValidationCommand -Name "TYPECHECK" -Command "npm run typecheck" -WorkingDirectory $RepoRoot -OutputDir $RunDir))
        }
        if (Has-NpmScript -PackageJson $PackageJson -ScriptName "lint") {
            $ValidationResults.Add((Invoke-ValidationCommand -Name "LINT" -Command "npm run lint" -WorkingDirectory $RepoRoot -OutputDir $RunDir))
        }
        if (Has-NpmScript -PackageJson $PackageJson -ScriptName "build") {
            $ValidationResults.Add((Invoke-ValidationCommand -Name "BUILD" -Command "npm run build" -WorkingDirectory $RepoRoot -OutputDir $RunDir))
        }
        if (Has-NpmScript -PackageJson $PackageJson -ScriptName "test") {
            $ValidationResults.Add((Invoke-ValidationCommand -Name "TEST" -Command "npm test" -WorkingDirectory $RepoRoot -OutputDir $RunDir))
        }
...
$ValidationRows = New-Object System.Collections.Generic.List[string]
if ($ValidationResults.Count -gt 0) {
    foreach ($V in $ValidationResults) {
        $ValidationRows.Add("| $($V.Name) | ``$($V.Command)`` | $($V.ExitCode) | $($V.Skipped) | ``$($V.OutputPath)`` |")
    }
} else {
    $ValidationRows.Add("| _No validation commands executed_ | - | - | - | - |")
}
$ValidationTable = $ValidationRows -join "`r`n"

$ScaffoldRows = New-Object System.Collections.Generic.List[string]
if ($ScaffoldFiles.Count -gt 0) {
    foreach ($S in $ScaffoldFiles) {
        $ScaffoldRows.Add("| ``$S`` |")
    }
} else {
    $ScaffoldRows.Add("| _No scaffold files created_ |")
}
...
$ReportTemplate = @'
# APEX WhatsApp Chat Behavior V3 Report
...
- V3 intentionally avoids StrictMode variable-expansion failures.
...
Write-Step "APEX WhatsApp Chat Behavior V3 finished."
Write-Host ""
Write-Host "DONE"
