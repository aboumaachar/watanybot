...
function Invoke-ValidationCommand {
    ...
    # Robust native-process runner:
    # npm may write warnings to STDERR even when the command should continue.
    # Using ProcessStartInfo prevents PowerShell from converting STDERR into a terminating NativeCommandError.
...
Set-Content -LiteralPath $LogPath -Value "APEX WhatsApp Chat Full Mode V3`r`nRunId: $RunId`r`nRepoRoot: $RepoRoot`r`n" -Encoding utf8

$ReportPath = Join-Path $RunDir "APEX_WHATSAPP_CHAT_FULLMODE_V3_REPORT.md"
...
Write-Step "APEX WhatsApp Chat Full Mode V3 started"
...
try {
    ...
    if (-not $SkipSelfInstall -and $SelfPath -and $SelfPath.StartsWith($Downloads, [System.StringComparison]::OrdinalIgnoreCase)) {
        ...
        $DestScript = Join-Path $ScriptDir "APEX-WhatsApp-Chat-FullMode-V3.ps1"
        if ($Apply) {
...
$Inventory = @"
# Active Chat File Inventory — Full Mode V3
...
$Report = @"
# APEX WhatsApp Chat Full Mode V3 Report
...
The previous validation runner treated npm STDERR warnings as fatal NativeCommandError. This version captures STDOUT/STDERR safely and continues to write the final report even when a validation command fails.
"@
...
Write-Step "APEX WhatsApp Chat Full Mode V3 finished."
Write-Host ""
Write-Host "DONE"
