...
.DESCRIPTION
  ...
  V1.0.1 permanently eliminates the V1.0.0 Windows PowerShell 5.1 native-argument
  transport failure class APEX_PS51_DOCKER_MULTILINE_SH_LC_NATIVE_ARGUMENT_TRANSPORT_CORRUPTION.
  No multiline shell program is passed as a docker CLI argument. Multiline helper
  programs are written to a temporary host file using UTF-8 without BOM + LF,
  bind-mounted read-only, and executed inside the helper container by a short,
  single-line argv chain.
...
$AuthorityId = 'APEX_WATANY_CONTROL_CENTER_STAGE_X_ERPNEXT_PHASE2_REDIS_RUNTIME_RECOVERY_EVIDENCE_FINALIZATION_PS51_NATIVE_ARGUMENT_TRANSPORT_REPAIR_AND_PRE_BACKEND_GATE_CLOSURE_V1_0_1'
...
function Get-ContainerField {
    ...
    $resultObject = Get-DockerResult -ArgumentVector @('inspect','--format',$Format,$ContainerName) -Label $Label
    return $resultObject.Stdout.Trim()
}
...
function Get-RedisRuntimeProof {
    ...
    $runningState = Get-ContainerField -ContainerName $ContainerName -Format '{{.State.Status}}' -Label ($ServiceName + '_STATE')
    $imageName = Get-ContainerField -ContainerName $ContainerName -Format '{{.Config.Image}}' -Label ($ServiceName + '_IMAGE')
    $networkNamesText = Get-ContainerField -ContainerName $ContainerName -Format '{{range $networkName,$networkValue := .NetworkSettings.Networks}}{{println $networkName}}{{end}}' -Label ($ServiceName + '_NETWORKS')
    $aliasText = Get-ContainerField -ContainerName $ContainerName -Format '{{range $networkName,$networkValue := .NetworkSettings.Networks}}{{range $networkValue.Aliases}}{{println .}}{{end}}{{end}}' -Label ($ServiceName + '_ALIASES')
    $healthText = Get-ContainerField -ContainerName $ContainerName -Format '{{if .State.Health}}{{.State.Health.Status}}{{else}}NOT_DEFINED{{end}}' -Label ($ServiceName + '_HEALTH')
    ...
    $networkLines = @(Get-Lines -Text $networkNamesText)
    ...
    $aliasLines = @(Get-Lines -Text $aliasText)
...
try {
    ...
    # V1.0.0 predecessor defect is registered before any substantive Docker proof.
    ...
    $dbState = Get-ContainerField -ContainerName $DbContainerName -Format '{{.State.Status}}' -Label 'DB_STATE_PRECHECK'
    $dbHealth = Get-ContainerField -ContainerName $DbContainerName -Format '{{if .State.Health}}{{.State.Health.Status}}{{else}}NOT_DEFINED{{end}}' -Label 'DB_HEALTH_PRECHECK'
    $dbImage = Get-ContainerField -ContainerName $DbContainerName -Format '{{.Config.Image}}' -Label 'DB_IMAGE_PRECHECK'
    $dbMounts = Get-ContainerField -ContainerName $DbContainerName -Format '{{range .Mounts}}{{.Name}}|{{.Destination}}|{{.RW}}{{println}}{{end}}' -Label 'DB_MOUNTS_PRECHECK'
    $dbNetworks = Get-ContainerField -ContainerName $DbContainerName -Format '{{range $networkName,$networkValue := .NetworkSettings.Networks}}{{println $networkName}}{{end}}' -Label 'DB_NETWORK_PRECHECK'
    ...
    if ($dbMounts.IndexOf($DbVolumeName,[System.StringComparison]::OrdinalIgnoreCase) -lt 0) {
        throw 'APEX_DB_PERSISTENT_VOLUME_NOT_MOUNTED'
    }
    if ($dbNetworks.IndexOf($ExpectedNetworkName,[System.StringComparison]::OrdinalIgnoreCase) -lt 0) {
    ...
    $queueMounts = Get-ContainerField -ContainerName $RedisQueueContainerName -Format '{{range .Mounts}}{{.Name}}|{{.Destination}}|{{.RW}}{{println}}{{end}}' -Label 'REDIS_QUEUE_MOUNTS'
    $queueVolumeProven = (
        ($queueMounts.IndexOf($RedisQueueVolumeName,[System.StringComparison]::OrdinalIgnoreCase) -ge 0) -and
        ($queueMounts.IndexOf('|/data|True',[System.StringComparison]::OrdinalIgnoreCase) -ge 0)
    )
    if (-not $queueVolumeProven) {
        Register-FailureClass -Class 'APEX_STAGE_X_PHASE2_REDIS_QUEUE_VOLUME_MOUNT_MISMATCH' -Detail $queueMounts
    ...
    $postRedisDbHealth = Get-ContainerField -ContainerName $DbContainerName -Format '{{if .State.Health}}{{.State.Health.Status}}{{else}}NOT_DEFINED{{end}}' -Label 'POST_REDIS_DB_HEALTH'
    ...
    $serviceCensusResult = Get-DockerResult -ArgumentVector @(
        ...
        '--format','{{.Names}}|{{.Image}}|{{.Status}}|{{.Label "com.docker.compose.service"}}'
    ) -Label 'RUNNING_SERVICE_CENSUS'

    $serviceRows = New-Object System.Collections.Generic.List[object]
    foreach ($serviceLine in (Get-Lines -Text $serviceCensusResult.Stdout)) {
        $serviceParts = $serviceLine.Split('|')
        if ($serviceParts.Count -lt 4) {
            throw ('APEX_SERVICE_CENSUS_OUTPUT_INVALID: {0}' -f $serviceLine)
        }
        $serviceName = $serviceParts[3]
        $allowedValue = ($AllowedServiceNames -contains $serviceName)
        if (-not $allowedValue) {
            $UnauthorizedRunningServiceCount++
        }
        $serviceRows.Add([pscustomobject]@{
            Container = $serviceParts[0]
            Image = $serviceParts[1]
            Status = $serviceParts[2]
            Service = $serviceName
