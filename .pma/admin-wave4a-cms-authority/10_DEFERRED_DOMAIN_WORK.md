...
| Documents | `public.documents` stores user/file assets and verification state; the CMS adapter is an administrative projection | Gateway PostgreSQL document repository/service | Clarify file delivery and retention; do not migrate uploads into Payload |
...
Users, authentication sessions, applications, approvals, marketplace transactions, job applications, CRM/ERM operational records, and runtime audit logs remain outside Payload and outside generic editorial CMS completion unless a separate architecture decision proves otherwise.
