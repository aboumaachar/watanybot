...
.DESCRIPTION
  ...
  V1.0.4 additionally eliminates:
    APEX_PS51_MANDATORY_ARRAY_PARAMETER_EMPTY_COLLECTION_BINDING

  Evidence CSVs such as FAILURES and WARNINGS are valid when they contain only
  headers and zero rows. Windows PowerShell 5.1 rejects an empty array passed to
  a mandatory array parameter unless empty collections are explicitly allowed.
  V1.0.4 therefore treats evidence-row arrays as intentionally empty-capable and
  validates that a header-only CSV remains parseable and evidence-complete.
...
$ScriptVersion = 'V1_0_4'
$AuthorityId = 'APEX_WATANY_CONTROL_CENTER_STAGE_X_ERPNEXT_PHASE2_REDIS_RUNTIME_RECOVERY_EVIDENCE_FINALIZATION_PS51_EMPTY_COLLECTION_BINDING_REPAIR_AND_PRE_BACKEND_GATE_CLOSURE_V1_0_4'
...
try {
    ...
    $emptyCollectionFailureClass = 'APEX_PS51_MANDATORY_ARRAY_PARAMETER_EMPTY_COLLECTION_BINDING'
    if ([System.IO.File]::Exists($FailureRegisterPath)) {
        $failureRegisterText = [System.IO.File]::ReadAllText($FailureRegisterPath)
        if ($failureRegisterText.IndexOf($emptyCollectionFailureClass,[System.StringComparison]::OrdinalIgnoreCase) -lt 0) {
            $registrationText = @(
                '',
                ('### {0}' -f $emptyCollectionFailureClass),
                '',
                ('- First formalized by: {0}' -f $AuthorityId),
                '- Predecessor: V1.0.3 Phase 2 evidence finalizer',
                '- Symptom: Windows PowerShell 5.1 rejected a valid zero-row evidence array with `Cannot bind argument to parameter Rows because it is an empty array.`',
                '- Permanent guard: evidence row-array parameters that semantically permit zero rows MUST declare AllowEmptyCollection and emit parseable header-only CSV.',
                '- Status: ACTIVE regression guard',
                ''
            ) -join [Environment]::NewLine
            $registrationEncoding = New-Object System.Text.UTF8Encoding($true)
        ...
        Add-GateRow -Gate 'PS51_EMPTY_COLLECTION_BINDING_REGRESSION_REGISTER' -Status 'PASS' -Detail $emptyCollectionFailureClass
    }
    else {
        Add-WarningRow -Class 'APEX_FAILURE_REGISTER_NOT_FOUND' -Detail $FailureRegisterPath
        Add-GateRow -Gate 'PS51_EMPTY_COLLECTION_BINDING_REGRESSION_REGISTER' -Status 'UNVERIFIED' -Detail $FailureRegisterPath
    }
    ...
    $emptyCollectionGuardToken = '[AllowEmptyCollection()][object[]]$Rows'
    if ($currentControllerText.IndexOf($emptyCollectionGuardToken,[System.StringComparison]::Ordinal) -lt 0) {
        throw 'APEX_PS51_MANDATORY_ARRAY_PARAMETER_EMPTY_COLLECTION_BINDING_REGRESSION_RECURRED'
    }
    Add-GateRow -Gate 'EMPTY_COLLECTION_BINDING_REGRESSION_SCAN' -Status 'PASS' -Detail 'Evidence Rows parameter explicitly accepts empty collections'
    ...
    Write-JsonFile -Name '01_APEX_CONTRACT_STATUS.json' -Value ([ordered]@{
        ...
        emptyCollectionBindingRepair = 'ALLOW_EMPTY_COLLECTION_HEADER_ONLY_CSV'
        zeroRowEvidenceCsvAllowed = $true
    })
    ...
    Write-JsonFile -Name '36_PS51_PARSER_PREFLIGHT.json' -Value ([ordered]@{
        ...
        emptyEvidenceRows = 'HEADER_ONLY_CSV'
        emptyCollectionBinding = 'EXPLICITLY_ALLOWED'
    })
    ...
    Write-Output 'PS51_EMPTY_COLLECTION_BINDING_REPAIR=PASS'
    Write-Output 'ZERO_ROW_EVIDENCE_CSV=SUPPORTED'
