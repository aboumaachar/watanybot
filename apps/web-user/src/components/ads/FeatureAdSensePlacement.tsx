...
const containerStyle = { margin: "16px 0 0", padding: "0 2px" } as const;
...
function featureAdSlot() {
  const featureSlot = String(import.meta.env.VITE_ADSENSE_FEATURE_SLOT_ID ?? "").trim();
  const sharedSlot = String(import.meta.env.VITE_ADSENSE_SALARY_SLOT_ID ?? "").trim();
  const raw = featureSlot || sharedSlot;
  return /^\d+$/.test(raw) ? raw : "9426086926";
}

function mobileFrameStyle(width: number) {
  return { width: "100%", maxWidth: `${width}px`, height: "100px", margin: "0 auto", overflow: "hidden", borderRadius: "14px" } as const;
}
...
export function FeatureAdSensePlacement({ featureId, placement = "bottom" }: { featureId: string; placement?: string }) {
  const slot = featureAdSlot();
  ...
  return (
    <aside data-feature-key={`feature.adsense.${featureId}.${placement}`} data-adsense-status="configured" aria-label="إعلان" style={containerStyle}>
