  ...
  return (
    <main className="ainelhafeh-page" dir="rtl">
      ...
      <section className="ainelhafeh-application" id="application">
        ...
        <form onSubmit={submit} className="ainelhafeh-form">
          ...
          <div className="ainelhafeh-question-grid">
            {[{ key: "arrive", label: "هل تستطيع الوصول إلى مفرق جبيل الاثنين 6:00؟" }, { key: "stand", label: "هل تستطيع العمل وقوفًا لعدة ساعات؟" }, { key: "experience", label: "هل لديك خبرة زراعية؟" }].map((question) => <fieldset key={question.key}><legend>{question.label}</legend><div className="ainelhafeh-options ainelhafeh-options--inline"><label><input required type="radio" name={question.key} checked={form[question.key as keyof FormState] === "نعم"} onChange={() => update(question.key as keyof FormState, "نعم")} /><span>نعم</span></label><label><input required type="radio" name={question.key} checked={form[question.key as keyof FormState] === "لا"} onChange={() => update(question.key as keyof FormState, "لا")} /><span>لا</span></label></div></fieldset>)}
          </div>
