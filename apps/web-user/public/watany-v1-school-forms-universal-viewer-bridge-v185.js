...
(function () {
  "use strict";
  var MARK = "watany-v1-school-forms-universal-viewer-bridge-v185";
  ...
  function isSchoolArea() {
    ...
    return path.indexOf("school") >= 0
      ...
      || !!document.querySelector(".school-grants-page, .school-aids-required-panel, #school-grants-forms, #school-grants-terms")
      || document.querySelector("#watany-school-forms-universal-bridge-panel");
  }

  function inSchoolScope(node) {
    if (!node || !node.closest) return false;
    return !!node.closest("#watany-school-forms-universal-bridge-panel, .school-grants-page, .school-aids-required-panel, #school-grants-forms, #school-grants-terms");
  }

  function refitAfterOpen() {
    var delays = [20, 80, 180, 360, 720, 1100];
    delays.forEach(function (delay) {
      setTimeout(function () {
        try {
          if (window.watanyUniversalFormViewer && typeof window.watanyUniversalFormViewer.fit === "function") {
            window.watanyUniversalFormViewer.fit();
          }
          document.documentElement.setAttribute("data-watany-school-form-viewer-refit", String(delay));
        } catch (err) {}
      }, delay);
    });
  }
  ...
  function wireExistingNodes() {
    if (!isSchoolArea()) return;
    var nodes = Array.prototype.slice.call(document.querySelectorAll("#watany-school-forms-universal-bridge-panel a, #watany-school-forms-universal-bridge-panel button, #watany-school-forms-universal-bridge-panel [role='button'], .school-grants-page a, .school-grants-page button, .school-grants-page [role='button'], .school-aids-required-panel a, .school-aids-required-panel button, .school-aids-required-panel [role='button'], #school-grants-forms a, #school-grants-forms button, #school-grants-forms [role='button'], #school-grants-terms a, #school-grants-terms button, #school-grants-terms [role='button'], .school-aid-form-card, .required-form-card"));
    nodes.forEach(function (node) {
      if (!inSchoolScope(node)) return;
  ...
  function init() {
    ...
    document.documentElement.setAttribute("data-watany-v1-school-forms-universal-bridge-version", "v1.8.5");
    window.watanyV1SchoolFormsUniversalViewerBridgeReady = true;
    window.watanyV1SchoolFormsUniversalViewerBridgeVersion = "v1.8.5";
