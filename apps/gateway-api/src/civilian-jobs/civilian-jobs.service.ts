...
export async function listCivilianOpportunities(
  filters: ListOpportunityFilters = {},
...
export async function getCivilianOpportunity(
  id: string,
  repository: CivilianJobsRepository = civilianJobsRepository,
...
export async function listCivilianOpportunitySources(
  repository: CivilianJobsRepository = civilianJobsRepository,
): Promise<OpportunitySource[]> {
  return repository.listSources();
}

export async function createCivilianOpportunityApplication(
  input: OpportunityApplicationInput,
  repository: CivilianJobsRepository = civilianJobsRepository,
): Promise<OpportunityApplicationRecord> {
  const opportunity = await getCivilianOpportunity(input.opportunityId, repository);
  if (!opportunity) throw new Error("Civilian opportunity was not found or is not published.");
  ...
  if (input.cvUrl) {
    let parsedUrl: URL;
    try { parsedUrl = new URL(input.cvUrl); } catch { throw new Error("CV URL must be a valid external URL."); }
    if (parsedUrl.protocol !== "https:") throw new Error("CV URL must use HTTPS.");
  }
...
export async function listCivilianOpportunityApplications(
  repository: CivilianJobsRepository = civilianJobsRepository,
): Promise<OpportunityApplicationRecord[]> {
